...
//        AlipayClient alipayClient = ...
        //构造sdk的客户端对象
        AlipayClient alipayClient = new DefaultAlipayClient(//获得初始化的AlipayClient
                AlipayConfig.URL, APP_ID, APP_PRIVATE_KEY, AlipayConfig.FORMAT,
                AlipayConfig.CHARSET, ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
        AlipayTradeWapPayRequest alipayRequest = new AlipayTradeWapPayRequest();//创建API对应的request
//        alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        alipayRequest.setNotifyUrl("http://tjxt-user-t.itheima.net/xuecheng/orders/paynotify");//在公共参数中设置回跳和通知地址,即下单后支付宝通过paynotify接口通知我们
//        alipayRequest.setNotifyUrl("http://domain.com/CallBack/notify_url.jsp");//在公共参数中设置回跳和通知地址
        alipayRequest.setBizContent("{" +
                "    \"out_trade_no\":\"202303100101021174\"," +//20230310010102012下面测试这个订单号的
                "    \"total_amount\":0.1," +
...
//    }
    ...
    //接收通知,支付宝支付之后主动通过这个接口返回订单信息。不需要我们自己查询了。调用这个接口是因为上面的alipaytest设置了setNotifyUrl。但是支付宝是外网，进入不了这个接口
